...
function findRealStlFiles() {
  const configured = process.env.LAYERLING_PERF_STL_DIR?.trim();
  if (!configured) {
    return [];
  }
  ...
  if (!existsSync(directory) || !statSync(directory).isDirectory()) {
    throw new Error(`LAYERLING_PERF_STL_DIR is not a directory: ${directory}`);
  }
...
function printReport(rows: BenchRow[]) {
  ...
  console.log("\nLayerling STL performance report");
  console.log("Tip: set LAYERLING_PERF_STL_DIR=C:\\path\\to\\stls to include real STL files.");
  console.table(table);
